# Parameter Estimation by Sequential Testing (PEST; Taylor & Creelman, 1967)
  ...
  for(calibration in 1:repetition_number) {
    ...
    while (trial <= max_trials) {
      ...
      if (!is.na(last_adj_dir) && adj_dir != last_adj_dir) {
        ...
        last_reversal <-trial
        step <- step/2
        doubled_before_rever <- doubled
      }
      
      doubled <- 0
      if ((trial - last_reversal>=3 && doubled_before_rever==0) ||trial - last_reversal>=4) {
        step <- step *2
        doubled <- 1
      }
