# Accelerated Stochastic Approximation method (ASA; Faes et al., 2007; Kesten, 1958)
...
run_single_calibration <- function(s_ind,
  ...
  # Loop through each calibration repetition
  for(calibration in 1:repetition_number) {
    ...
    while (trial <= max_trials) {
      ...
      if (trial >1){
        step <- (initial_step/ (1+num_of_reversals)) *0.5  #ASA rule
